...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@agentops/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@agentops/normalization-core/string-coerce"),
    )?.toEqual({
      find: "@agentops/normalization-core/string-coerce",
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex((alias) => alias.find === "@agentops/net-policy/ip");
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@agentops/net-policy",
    );
    ...
    const broadAgentOpsWildcardIndex = aliases.findIndex(
    ...
    expect(broadAgentOpsWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadAgentOpsWildcardIndex);
  });
