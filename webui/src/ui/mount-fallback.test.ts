...
async function readIndexHtmlWithDelay(delayMs: number): Promise<string> {
  ...
  return html.replace(
    'data-agentops-mount-timeout-ms="12000"',
    `data-agentops-mount-timeout-ms="${delayMs}"`,
  );
}
...
function installFallbackShell(window: TestWindow, html: string): void {
  ...
  const sentinel = Array.from(parsed.querySelectorAll<HTMLScriptElement>("script:not([src])")).find(
    (script) => script.textContent?.includes("agentops-mount-fallback"),
  );
...
describe("Control UI mount fallback", () => {
  ...
  it("shows the static troubleshooting panel when the app element is never registered", async () => {
    ...
    expect(frameWindow.customElements.get("agentops-app")).toBeUndefined();
    ...
    const fallback = requireElementById(
      frameWindow,
      "agentops-mount-fallback",
      frameWindow.HTMLElement,
    );
    expect(fallback.hidden).toBe(false);
    expect([...frameWindow.document.body.classList]).toEqual(["agentops-mount-fallback-active"]);
  ...
  it("keeps the fallback hidden when the app element registers before the timeout", async () => {
    ...
    if (!frameWindow.customElements.get("agentops-app")) {
      frameWindow.customElements.define("agentops-app", class extends frameWindow.HTMLElement {});
    }
    await frameWindow.customElements.whenDefined("agentops-app");
