...
declare global {
  interface Window {
    __AGENTOPS_NATIVE_CONTROL_AUTH__?: {
...
function applyNativeControlAuth(host: SettingsHost) {
  const nativeAuth = window["__AGENTOPS_NATIVE_CONTROL_AUTH__"];
  if (!nativeAuth) {
    return;
  }
  try {
    delete window["__AGENTOPS_NATIVE_CONTROL_AUTH__"];
  } catch {
    window["__AGENTOPS_NATIVE_CONTROL_AUTH__"] = undefined;
  }
...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[agentops] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
export function inferBasePath() {
  ...
  const configured = window["__AGENTOPS_CONTROL_UI_BASE_PATH__"];
