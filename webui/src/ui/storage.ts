...
const SETTINGS_KEY_PREFIX = "agentops.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "agentops.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "agentops.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "agentops.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "agentops.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "agentops.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__AGENTOPS_CONTROL_UI_BASE_PATH__"]);
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("agentops.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
