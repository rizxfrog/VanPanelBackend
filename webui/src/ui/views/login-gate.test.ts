...
describe("resolveLoginFailureFeedback", () => {
  ...
  it("explains missing auth credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Paste the token from agentops dashboard --no-open or enter the configured password.",
      "If no token is configured, run agentops doctor --generate-gateway-token on the gateway host.",
  ...
  it("explains rejected stale credentials", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run agentops dashboard --no-open and open the fresh URL or paste its token.",
  ...
  it("preserves pairing request ids in the approval command", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Run agentops devices list on the Gateway host.",
      "Approve this request: agentops devices approve req-123.",
  ...
  it("keeps generic browser WebSocket constructor failures on the network path", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Confirm the Gateway is running with agentops status or agentops gateway run.",
      "Check the WebSocket URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
      "Reopen the dashboard with agentops dashboard --no-open to recopy the current URL and auth details.",
    ]);
  });
  ...
  it("explains protocol mismatch without requiring a gateway protocol change", () => {
    ...
    expect(feedback?.steps).toEqual([
      "Reopen the served dashboard with agentops dashboard so the UI and Gateway come from the same install.",
      "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
      "Restart the Gateway after updating AgentOps so it serves the current protocol.",
    ]);
  });
  ...
  it("redacts credential-shaped values from displayed raw errors", () => {
    const feedback = resolveLoginFailureFeedback({
      connected: false,
      lastError:
        "failed ws://host/agentops#token=secret-token Authorization: Bearer secret-bearer token=inline-secret",
      lastErrorCode: null,
      hasToken: false,
      hasPassword: false,
    });
    ...
    expect(feedback?.rawError).toBe(
      "failed ws://host/agentops#[redacted-credential] Authorization: Bearer [redacted] token=[redacted]",
    );
  });
});
