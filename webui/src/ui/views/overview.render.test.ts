...
describe("overview view rendering", () => {
  ...
  it("renders a dedicated scope-upgrade approval hint with the exact approve command", async () => {
    ...
    expect(compactText(hint)).toBe(
      "Scope upgrade pending approval. This device is already paired, but the requested wider scope is waiting for approval. agentops devices approve req-123 agentops devices list On mobile? Copy the full URL (including #token=...) from agentops dashboard --no-open on your desktop. Docs: Device pairing",
    );
    expect([...container.querySelectorAll(".mono")].map((node) => node.textContent)).toEqual([
      "agentops devices approve req-123",
      "agentops devices list",
    ]);
  });
  ...
  it("does not suggest preview-only latest approval when the request id is absent", async () => {
    ...
    const hint = container.querySelector(".mono")?.closest(".muted") ?? null;
    expect(compactText(hint)).toBe(
      "Scope upgrade pending approval. This device is already paired, but the requested wider scope is waiting for approval. agentops devices list On mobile? Copy the full URL (including #token=...) from agentops dashboard --no-open on your desktop. Docs: Device pairing",
    );
    expect([...container.querySelectorAll(".mono")].map((node) => node.textContent)).toEqual([
      "agentops devices list",
    ]);
