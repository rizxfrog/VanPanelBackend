...
describeControlUiE2e("Control UI cron mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set AGENTOPS_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
    ...
    const gateway = await installMockGateway(page, {
      methodResponses: {
        ...
        "cron.status": {
          ...
          storePath: "/tmp/agentops-e2e/cron/jobs.json",
        },
      },
    });
