...
function resolvePendingUpdateHandoffTimeoutBanner(): {
  ...
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `agentops update status` for the final result.",
  };
}
...
export function connectGateway(host: GatewayHost, options?: ConnectGatewayOptions) {
  ...
  const client = new GatewayBrowserClient({
    ...
    clientName: "agentops-control-ui",
    clientVersion,
    mode: "webchat",
      ...
      if (host.pendingAbort) {
        ...
        void host.client
          ...
          .catch((err: unknown) => {
            ...
            console.warn("[agentops] pending abort failed:", err);
          });
      }
