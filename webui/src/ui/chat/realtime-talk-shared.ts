...
function getTerminalAgentWaitError(result: AgentWaitResult | undefined): Error | undefined {
  ...
  if (result.status === "error") {
    return new Error(message || "AgentOps tool call failed");
  }
  ...
  if (hasTerminalTimeoutMetadata) {
    return new Error(message || "AgentOps tool call timed out");
  }
  return undefined;
}
...
function waitForChatResult(params: {
  ...
  return new Promise((resolve, reject) => {
    if (params.signal?.aborted) {
      reject(new DOMException("AgentOps tool call aborted", "AbortError"));
      return;
    }
    const timer = window.setTimeout(() => {
      settleReject(new Error("AgentOps tool call timed out"));
    }, params.timeoutMs);
    let settled = false;
    let emptyFinalWaitStarted = false;
    let emptyFinalFallbackTimer: number | undefined;
    const onAbort = () => {
      settleReject(new DOMException("AgentOps tool call aborted", "AbortError"));
    };
      ...
      void params.client
        ...
        .then((result) => {
          ...
          emptyFinalFallbackTimer = window.setTimeout(() => {
            settleResolve("AgentOps finished with no text.");
    ...
    unsubscribe = params.client.addEventListener((evt: GatewayEventFrame) => {
      ...
      if (payload.state === "final") {
        ...
        settleReject(
          new DOMException(payload.errorMessage ?? "AgentOps tool call aborted", "AbortError"),
        );
      } else if (payload.state === "error") {
        settleReject(new Error(payload.errorMessage ?? "AgentOps tool call failed"));
      }
    });
    function cleanup() {
...
export async function submitRealtimeTalkConsult(params: {
  ...
  try {
    ...
    if (!runId) {
      throw new Error("AgentOps realtime tool call did not return a run id");
    }
    if (params.signal?.aborted) {
      abortRun();
      submitAbortResult();
