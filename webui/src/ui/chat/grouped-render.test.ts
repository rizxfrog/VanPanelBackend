...
vi.mock("../views/agents-utils.ts", () => {
  ...
  return {
    assistantAvatarFallbackUrl: () => "/agentops-molty.png",
    agentLogoUrl: () => "/agentops-logo.svg",
...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__agentops__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("agentops:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it("renders allowed transcript and content image variants", async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      expect(mediaUrl.pathname).toBe("/agentops/__agentops__/assistant-media");
      expect([...mediaUrl.searchParams.keys()].toSorted()).toEqual(["meta", "source"]);
      expect(mediaUrl.searchParams.get("meta")).toBe("1");
      expect(mediaUrl.searchParams.get("source")).toMatch(/^\/tmp\/agentops\/.+\.(png|jpg)$/u);
    ...
    const renderUserMedia = (message: unknown) => {
      ...
      const renderMessage = () =>
        renderGroupedMessage(container, message, "user", {
          ...
          localMediaPreviewRoots: ["/tmp/agentops"],
    ...
    let container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/tmp/agentops/user-upload.png",
      timestamp: Date.now(),
    });
    ...
    expect(
      ...
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Fuser-upload.png&mediaTicket=ticket-user",
    );
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPaths: ["/tmp/agentops/first.png", "/tmp/agentops/second.jpg"],
    ...
    expect(
      ...
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ffirst.png&mediaTicket=ticket-user",
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Fsecond.jpg&mediaTicket=ticket-user",
    ]);
    ...
    container = renderUserMedia({
      ...
      role: "user",
      content: "",
      MediaPath: "/__agentops__/media/user-upload.pdf",
    ...
    expect(documentLink?.getAttribute("href")).toBe("/__agentops__/media/user-upload.pdf");
    vi.unstubAllGlobals();
  });
  ...
  it("fetches managed chat images with auth and renders blob previews", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const headers = init?.headers as Headers;
      expect(headers.get("Authorization")).toBe("Bearer session-token");
      expect(headers.get("x-agentops-requester-session-key")).toBe("agent:main:main");
      return {
        ok: true,
  ...
  it("renders verified local assistant attachments through the Control UI media route", async () => {
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        container,
        {
          ...
          role: "assistant",
          content:
            "Local image\nMEDIA:/tmp/agentops/test image.png\nMEDIA:/tmp/agentops/test-doc.pdf",
          timestamp: Date.now(),
        },
        {
          showToolCalls: false,
    ...
    const [, fetchInit] = requireFetchCallForUrl(
      fetchMock,
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&meta=1",
    );
    expectSameOriginGet(fetchInit);
    ...
    expect(image?.getAttribute("src")).toBe(
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&mediaTicket=ticket-local",
    );
    expect(docLink?.getAttribute("href")).toBe(
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest-doc.pdf&mediaTicket=ticket-local",
    );
  ...
  it("refreshes cached local assistant media tickets before they expire without another render", async () => {
    ...
    expect(
      ...
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&mediaTicket=ticket-old",
    );
    ...
    expect(
      ...
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&mediaTicket=ticket-new",
    );
    vi.useRealTimers();
    vi.unstubAllGlobals();
  });
  ...
  it("rechecks local assistant attachment availability when the auth token changes", async () => {
    ...
    expect(image.getAttribute("src")).toBe(
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&mediaTicket=ticket-fresh",
    );
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__agentops__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(docLink?.getAttribute("href")).toBe("/__agentops__/media/test-doc.pdf");
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
    ...
    const cases = [
      renderCase({
        ...
        message: {
          ...
          role: "assistant",
          content: "Windows image\nMEDIA:file:///C:/tmp/agentops/test%20image.png",
          timestamp: Date.now(),
        },
        expectedUrl:
          "/agentops/__agentops__/assistant-media?source=%2FC%3A%2Ftmp%2Fagentops%2Ftest%2520image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/agentops/__agentops__/assistant-media?source=C%3A%5CUsers%5CTest%5CPictures%5Ctest+image.png&meta=1",
      }),
      renderCase({
        ...
        expectedUrl:
          "/agentops/__agentops__/assistant-media?source=%7E%2FPictures%2Ftest+image.png&meta=1",
      }),
    ];
  ...
  it("revalidates cached unavailable local assistant attachments after retry window", async () => {
    ...
    expect(
      ...
      "/agentops/__agentops__/assistant-media?source=%2Ftmp%2Fagentops%2Ftest+image.png&mediaTicket=ticket-retry",
    );
    expect(container.querySelector(".chat-assistant-attachment-badge")).toBeNull();
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__agentops__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasPluginSurfaceUrl: "http://127.0.0.1:19003/__agentops__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__agentops__/cap/cap_123/__agentops__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
  ...
  it("renders server-history canvas blocks for the live toolResult sequence after history reload", () => {
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__agentops__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__agentops__/canvas/documents/cv_canvas_live_history/index.html",
    );
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__agentops__/canvas/documents/cv_inline_default/index.html",
    );
  ...
  it("renders assistant_message canvas results in the assistant bubble even when tool rows are visible", () => {
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__agentops__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
    ...
    const iframe = expectElement(bubble, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__agentops__/canvas/documents/cv_inline_visible/index.html",
    );
  ...
  it("adds a full-message request when opening a truncated assistant message", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __agentops: { id: "msg-truncated-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
  ...
  it("does not add a full-message request for non-truncated assistant messages", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __agentops: { id: "msg-visible-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
  ...
  it("does not add a full-message request for mirrored message-tool replies", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        agentopsMessageToolMirror: { toolName: "message", toolCallId: "call-1" },
        __agentops: { id: "msg-tool-result", seq: 2, truncated: true },
      },
      {
        sessionKey: "global",
        agentId: "work",
