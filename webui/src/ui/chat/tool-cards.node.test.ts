...
*No output — tool failed.*`);
  ...
  it("extracts canvas handle payloads into canvas previews", () => {
    const [card] = extractToolCards(
      {
        ...
        content: JSON.stringify({
          ...
          view: {
            backend: "canvas",
            id: "cv_inline",
            url: "/__agentops__/canvas/documents/cv_inline/index.html",
          },
          presentation: {
    ...
    expect(card?.preview?.url).toBe("/__agentops__/canvas/documents/cv_inline/index.html");
    ...
    const [card] = extractToolCards(
      {
        ...
        __agentops: { id: "msg-tool-history-1", seq: 7 },
      },
      "msg:history",
    );
    ...
    const cases = [
      {
        ...
        content: JSON.stringify({
          ...
          view: {
            backend: "canvas",
            id: "cv_tool_card",
            url: "/__agentops__/canvas/documents/cv_tool_card/index.html",
          },
          presentation: {
            target: "tool_card",
