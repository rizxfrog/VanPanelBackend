...
import type { AgentOpsModalDialog } from "../ui/components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<AgentOpsModalDialog>("agentops-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected agentops-modal-dialog");
  }
